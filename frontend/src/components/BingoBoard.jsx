...
const shuffleArray = (array) => {
 ...
 const playAmharicAudioForNumbermemale = (number) => {
             ...
             for (let i = 0; i < 5; i++) {
                if (initialState[i].every((num) => calledNumbers.includes(num) || num === '*')) {
                    ...
                    if (initialState[i].includes(lastCalledNumber)) {
                        ...
                        if(coinawre>=100 || numberCallLength<=10){
                          setTriggerSpin(true);
    ...
    // If the number is not in the cartela at all
    if (!cartelas.flat().includes(num)) {
        if (language == "am") {
            gameisnot();
        } else if (language == "amf") {
    ...
    // ✅ Check if every number (excluding "*") is marked
    const allMarked = initialState.flat().every(cell =>
        cell === '*' || calledNumbers.includes(cell)
    );

    if (allMarked) {
        return declareWin("Congratulations! All numbers on your card are marked!", initialState.flat());
    }

    // ❌ Not all numbers marked — no win
    notifyNoWin(initialState);
};
